...
대안이 `한글로 먼저 적고 괄호 안에 넣기` 같은 지시문이거나 `(으)로 / ~하여` 처럼 앞말에 따라
형태가 달라지면 기계가 고를 수 없습니다(`isApplicable()`). 이런 항목에는 지적 카드 안에
**직접 적어 넣는 칸**이 뜨고, 적으면 그 말이 수정본에 들어갑니다.
규칙 258개 중 이런 것은 11개(전부 번역 투)이고, 나머지는 답이 하나라 그대로 반영됩니다.
