...
// 규칙 검사와 같은 숫자를 쓴다 — 한 도구가 두 소리를 내면 안 된다
check('기준이 하나다', LONG_SENTENCE, 40);
const 긴글 =
  '○ 장기영 미래교육과장은 “‘K뚝배기’는 정보 유출 걱정 없이 자체 자원으로 행정 병목을 뚫어낸 ' +
  '인공지능(AI) 혁신 사례”라며 “문서 담당자의 개입 없이 신속·정확하게 공문을 배부하는 자동화 ' +
  '체계를 통해 단순 반복 업무를 없애고, 행정력 낭비를 줄여 교직원 업무 경감에 큰 도움이 될 ' +
  '것”이라고 말했다.';
