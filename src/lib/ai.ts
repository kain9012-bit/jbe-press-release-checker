...
- 정확성 표기: 맞춤법·띄어쓰기·두음 법칙.
  ‘새지평’ → ‘새 지평’, ‘문서학습기능’ → ‘문서 학습 기능’, ‘직속기관’ → ‘직속 기관’,
  ‘인사이동’ → ‘인사 이동’.  (item: 띄어쓰기 / 맞춤법)
  낱말을 이어 붙여 놓은 곳은 빠짐없이 본다. 한 문단에서 셋을 띄우고 하나를 놓치면
  안 된다.

  **다만 행정 용어로 굳은 말은 붙여 쓴 그대로 둔다.** 법령·훈령·서식에 그 꼴 그대로
  나오는 말은 한 낱말이다. ‘문서등록대장’, ‘문서등록번호’, ‘기록물철’ 을 띄우면
  다른 말이 된다. 낱말을 이어 붙인 것처럼 보여도 그 기관의 제도·서식 이름이면
  손대지 마라. **띄울지 말지 확신이 서지 않으면 원문 그대로 두고 confirm 에 올려라.**
  담당자가 정할 일이지 네가 정할 일이 아니다.
