...
- 용이성 ② 쉬운 우리말: 어려운 외래어·행정 용어를 쉬운 말로. 필요하면 병기한다.
  ‘클라우드’ → ‘구름 망(클라우드)’.  (item: 쉬운 우리말)
  **다만 국어사전에 오른 말은 그대로 둔다.** ‘시스템’, ‘프로그램’, ‘서비스’, ‘센터’ 처럼
  이미 우리말처럼 쓰는 낱말은 고치지 않는다. 같은 원고를 두 번 넣었을 때 한 번은
  ‘시스템’, 한 번은 ‘체계(시스템)’ 가 나오면 담당자는 무엇을 믿어야 할지 모른다.
- **고치기로 했으면 한 가지 꼴로만 고친다.** ‘점검해’ 로 고칠 것인지
  ‘점검해(모니터링해)’ 로 고칠 것인지 원고 안에서 오락가락하지 마라.
  같은 낱말은 원고 어디에서나 같은 말로 고친다.
- 정확성 표기: 맞춤법·띄어쓰기·두음 법칙.
  ‘새지평’ → ‘새 지평’, ‘문서학습기능’ → ‘문서 학습 기능’.  (item: 띄어쓰기 / 맞춤법)
