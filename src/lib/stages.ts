...
export const STAGE3: StageSpec = {
  ...
  does: '잘못 고친 자리를 찾아 그 자리에 들어갈 더 나은 말을 낸다.',
  never: [
    '새로 고칠 곳을 찾는 것 — 그건 2차가 이미 했다',
    '묻지 않은 자리를 답하는 것',
    '취향으로 트집 잡는 것 — 규범에 어긋난 것만 본다',
    '무엇이 잘못이었는지 화면에 늘어놓는 것 — 담당자가 받을 것은 읽을거리가 아니라 고쳐진 글이다',
    '고치기 전으로 되돌리는 것 — 규범에 걸린 자리는 그대로 둘 수 없다. 지적을 없애는 것은 더더욱 안 된다',
  ],
  gives: '잘못 고친 자리의 id 와 그 자리에 넣을 더 나은 말',
};
 ...
 *
 * **고치기 전 말을 그대로 돌려주는 답도 버린다.** 한때는 그것을 ‘손대지 말라는 뜻’ 으로
 * 받아 그 지적을 없앴는데, 그러면 규범에 걸린 자리가 고쳐지지도 않고 목록에서도 사라진다.
 * 실제로 부제의 ‘꿈 UP! 미래 UP!’ 이 그렇게 통째로 없어졌다. 되돌리기는 3차의 일이
 * 아니다. 답을 버리면 앞 단계의 고침이 그대로 남고, 지적도 그대로 남는다.
 *
