...
export const STAGE3: StageSpec = {
  ...
  does: '고침 하나하나를 그 문맥에서 판정한다. 다르게 고쳐야 하면 옳은 말을 내고, 손대지 말았어야 할 자리면 고치기 전 말을 그대로 낸다.',
  never: [
    '새로 고칠 곳을 찾는 것 — 그건 2차가 이미 했다',
    '묻지 않은 자리를 답하는 것',
    '취향으로 트집 잡는 것 — 규범에 어긋난 것만 본다',
    '무엇이 잘못이었는지 화면에 늘어놓는 것 — 담당자가 받을 것은 읽을거리가 아니라 고쳐진 글이다',
    '지적을 없애는 것 — 그대로 두라고 판정해도 그 자리는 목록에 남는다. 고칠지 말지는 담당자가 본다',
  ],
  gives: '자리마다 그 문맥에서 들어가야 할 말 (그대로 두는 것이 맞으면 고치기 전 말)',
};
 ...
 *
 * **‘그대로 두라’ 는 답도 받는다.** 규칙은 낱말만 보고 ‘K’ 를 ‘케이(K)’ 로 바꾸지만,
 * 그것이 시스템 이름 ‘K뚝배기’ 의 첫 글자인지 아닌지는 문맥을 읽어야 안다. 그 판단은
 * 사람이 사전에 손으로 적어 둘 수 있는 것이 아니라 이 단계가 할 일이다.
 *
 * 한때는 이 답을 막아 두었다. 앞서 ‘그대로 두라’ 를 ‘그 지적을 없애라’ 로 읽은 탓에
 * 부제의 ‘꿈 UP! 미래 UP!’ 이 통째로 사라졌기 때문이다. 그건 답이 아니라 그 답을
 * 쓰는 쪽이 잘못이었다. **그대로 두는 것과 지적을 없애는 것은 다른 일이다.**
 * 지금은 그 자리를 꺼 두기만 하고 지적은 목록에 남긴다.
 *
...
export function guard3(
  ...
  for (const w of raw ?? []) {
    ...
    if (!fix) continue;
    // ‘그대로 두라’ — 규약 검사를 할 것이 없다. 그대로 넘겨서 그 자리를 끄게 한다.
    if (fix === from) {
      out[id] = fix;
      continue;
    }
